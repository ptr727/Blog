...
def exif_orientation(segment: bytes) -> int | None:
    ...
    An Orientation in or reached from an IFD whose entries run past the segment, or an IFD0 inside the TIFF header,
    is None too, since decoders differ on whether they read that IFD at all.
    """
    ...
    try:
        ...
        return 1
    # Each pending IFD carries whether a malformed IFD led to it.
    pending, seen = [(ifd0, False)], set()
    values, placed = set(), False
    while pending and len(seen) < 16:
        offset, tainted = pending.pop()
        ...
        tainted = tainted or offset < 8 or offset + 2 + count * 12 > len(raw)
        for index in range(count):
            entry = offset + 2 + index * 12
            if entry + 2 > len(raw):
                break
            tag = struct.unpack_from(fmt + "H", raw, entry)[0]
            if tag == 0x0112 and tainted:
                return None
            if entry + 12 > len(raw):
                break
            if tag == 0x0112:
                ...
                pending.append((pointer, tainted))
