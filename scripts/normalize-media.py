...
def normalize_png(data: bytes) -> bytes | None:
    ...
    A file whose Exif Orientation is not a single SHORT, or is one that turns the picture, is refused.
    """
...
def normalize_jpeg(data: bytes) -> bytes | None:
    ...
    for (marker, start, end), name in zip(parts, names):
        ...
        if name == b"ICC_PROFILE\x00":
            ...
            # Orientation decides how the picture displays, so any value but upright is re-emitted alone.
