...
# The selection cannot be reimplemented in shell: the test is that ONE item carries both post_type=post and status=publish, where two greps over a whole file would accept the media-only export.
set -Eeuo pipefail
...
# Resolved to absolute before anything else, because this script cd's into it and the export path is worked out beforehand.
# A relative CAPTURE_ROOT would yield a relative export path that stops resolving the moment the cd happens, which reads as a missing export rather than as a path problem.
