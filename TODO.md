...
- **Finish [#33][issue-33] by deploying production once.** Three of its four checks passed against the real host on 2026-08-05, and the evidence is on the issue: staging deployed end to end through the pinned transport, the stored `DEPLOY_SSH_KNOWN_HOSTS` verified against the live host key on both environments, `BatchMode` proven not to fight the forced command, and the production ref gate confirmed to refuse a dispatch from a non-default branch. The host side answered the rest, including that the runner reaches sshd directly and that one key is installed rather than the former pair. What remains is the fourth check, which is a production deploy, and that is the item two lines below rather than a transport question.
