...
- **Gallery captions render as a stray text node inside the grid** rather than as a caption, which is [#52][issue-52]. Twelve occurrences across two pages, from one source file, and it is a conversion artifact rather than anything the recent content work introduced. A fix has to decide whether the caption belongs to the last image or to the set, and the provenance capture is the only source for what the old platform actually displayed.
...
[issue-52]: https://github.com/ptr727/Blog/issues/52
