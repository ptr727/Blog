...
**The off-host copy, maintained outside this repository, writes the rotated access logs to `LOG_ARCHIVE_ROOT`, and this repository depends on nothing else about it.** Its schedule, what else it copies, its pull-versus-push direction, and its trust model are the backup host's own configuration to document, not this repository's. Read the unit and its last run on the backup host rather than trusting a schedule written down anywhere, including here.
