...
The mechanism is the `X-Blog-Check` request header, which every synthetic request announces itself with, so `jq 'select(.["request_X-Blog-Check"] == null)'` is the whole filter. Its value is `<source>/<id>` rather than a boolean, so a run is identifiable rather than merely excludable: `github/<run-id>-<attempt>` from CI, `vps/smoke` from the host side, `proxmox/<what>` from here.
