...
| `CHECK_TAG` | the `X-Blog-Check` provenance this run announces on every request, as `<source>/<id>`. Rarely set by hand: `check-live-urls.sh` derives `github/<run-id>-<attempt>` under Actions and `proxmox/manual` elsewhere. Set it to name a purpose for a hand run, as `proxmox/media-dev` |
